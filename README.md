...
For training traditional machine learning, we picked `Random Forest`, `Logistic Regression`, `Decision Tree`, Naive Bayes, `K Nearest Neighbor`, and `linear Support Vector Machine` for trianing data to compare the results of them. We chose those models due to their simplicity and fast of training.
